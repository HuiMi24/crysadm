...
def getaward_id(user_id):
    ...
    if r.get('rd') != 'ok':
        ...
        session['info_message'] = '获得:%s  下次转需要:%s秘银.<br />' % (
            regular_html(r.get('tip')), r.get('cost'))
...
def getaward_all():
    ...
    for b_user_id in r_session.smembers('accounts:%s' % username):
        ...
        if r.get('rd') != 'ok':
            ...
            red_log('手动执行', '转盘', user_id, '获得:%s' %
                    regular_html(r.get('tip')))
            account_data_key = account_key + ':data'
            account_data_value = json.loads(
                r_session.get(account_data_key).decode("utf-8"))
            account_data_value.get('mine_info')['td_not_in_a'] = 0
...
def searcht_all():
    ...
    for b_user_id in r_session.smembers('accounts:%s' % username):
        ...
        r = check_searcht(cookies)
        if r.get('r') != 0:
            error_message += 'Id:%s : %s<br />' % (
                user_id, regular_html(r.get('rd')))
            red_log('手动执行', '进攻', user_id, regular_html(r.get('rd')))
        else:
...
def stop_device():
    ...
    ubus_cd(session_id, account_id, 'check', [
            "dcdn", "stop", {}], '&device_id=%s' % device_id)
...
def start_device():
    ...
    ubus_cd(session_id, account_id, 'check', [
            "dcdn", "start", {}], '&device_id=%s' % device_id)
...
def upgrade_device():
    ...
    ubus_cd(session_id, account_id, 'get_progress', [
            "upgrade", "start", {}], '&device_id=%s' % device_id)
    ubus_cd(session_id, account_id, 'get_progress', [
            "upgrade", "get_progress", {}], '&device_id=%s' % device_id)
...
def reboot_device():
    ...
    ubus_cd(session_id, account_id, 'reboot', [
            "mnt", "reboot", {}], '&device_id=%s' % device_id)
...
def reset_device():
    ...
    ubus_cd(session_id, account_id, 'reset', [
            "mnt", "reset", {}], '&device_id=%s' % device_id)
...
def enable_upnp():
    ...
    ubus_cd(session_id, account_id, 'set_upnp', [
            "dcdn", "set_upnp", {"enabled": True}], '&device_id=%s' % device_id)
...
def disable_upnp():
    ...
    ubus_cd(session_id, account_id, 'set_upnp', ["dcdn", "set_upnp", {
            "enabled": False}], '&device_id=%s' % device_id)
...
def noblink_device():
    ...
    for i in range(10):  # 循环20次
        ubus_cd(session_id, account_id, 'noblink', [
                "mnt", "noblink", {}], '&device_id=%s' % device_id)  # 闪
        time.sleep(2)
        ubus_cd(session_id, account_id, 'blink', [
                "mnt", "blink", {}], '&device_id=%s' % device_id)  # 不闪
...
def admin_device():
    ...
    dev = ubus_cd(session_id, account_id, 'get_device', [
                  "server", "get_device", {"device_id": device_id}])
